    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between px-2 mb-4">
        <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
          <Share2 size={14} className="text-indigo-500" /> Naija Trends
        </h3>
      ...
      <div className="space-y-3">
        {posts.map((post, idx) => (
          <div key={idx} className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm hover:border-indigo-100 transition-all group">
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  {post.isVerified && <BadgeCheck size={14} className="text-indigo-500 fill-current text-white" />}
                ...
                <div className="flex items-center justify-between text-slate-400">
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex items-center gap-1.5 opacity-50">
                      ...
                      {post.sentiment === 'advice' && <Info size={14} className="text-indigo-500" />}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="bg-slate-900 p-6 rounded-[2rem] text-white/90 relative overflow-hidden group">
        ...
        <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-2">Market Sentiment</p>
        <p className="text-xs font-medium leading-relaxed">
          Monitor the feed! Rumors on "Naija X" often move the stock prices in the following week. 
          <span className="block mt-2 font-black text-white underline">#NairaWise #InvestmentTips</span>
        </p>
      </div>
    </div>
  );
